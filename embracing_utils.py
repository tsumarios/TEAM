...
This module provides utility functions related to semantic similarity computation, synset relations, and text analysis.
It leverages various NLP tools, including WordNet (via NLTK), spaCy, and Sentence Transformers, to process and analyze sentences. 
"""
...
def compute_semantic_similarity_scores(in_path: str, out_path: str) -> tuple:
    ...
    ordered_sentences = list(
        itertools.combinations(sentence_list, 2)
    )  # (n(n-1))/2 pairs, where n = len(sentence_list)
...
def compute_semantic_similarity_scores_between_files(
    in_path1: str, in_path2: str, out_path: str
) -> tuple:
    """
    Compute semantic similarity scores for each pair of sentences in the given input file.
    """
    sentence_list1 = pd.read_csv(in_path1, index_col="ID")["Threat"].values.tolist()
    sentence_list2 = pd.read_csv(in_path2, index_col="ID")["Threat"].values.tolist()
...
def merge_csv_files(file1, file2, output_file):
    """
    Merge input threats with semantic similarity score to add identifiers.
    """
...
def filter_dataframe_by_threshold(
...
def find_synset_relations(sentence1: str, sentence2: str) -> tuple:
    ...
    nouns2 = list(
        set([token.lemma_ for token in nlp(sentence2) if token.pos_ == "NOUN"])
    )
    for term1 in nouns1:
        for term2 in nouns2:
            if term1 != term2:
