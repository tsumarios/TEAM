...
with st.expander("📖 Tutorial/Guide"):
    st.markdown(
        ...
        This application helps automate threat embracing within the SPADA methodology by leveraging semantic analysis to identify and group related threats. Here’s a quick guide to get started:

        #### 1. Uploading Input Files
        - **Threats List**: Upload a CSV file containing the list of threats.
        - **Semantic Similarity Scores**: Upload a CSV file with pairwise similarity scores for each threat tuple. This file is generated by separate scripts, which vary depending on your chosen TEAM version:
          - **TEAM 2**: Use the `ss_scores_by_groups.py` script to compute scores for tuples with higher cardinality (e.g., triplets, quadruplets). The file will be stored as `./data/{input_threats_filename}_ss_scores_with_cardinality_{k}.csv`.
          - **TEAM 3**: Use the `ss_scores.py` script for pairwise comparisons. The file will be saved as `./data/{input_threats_filename}_ss_scores.csv`.

        #### 2. Configuring Threat Embracing Settings
        ...
        #### 3. Reviewing Embraceable Candidates
        ...
        - View detailed semantic relationships between the selected threats, including part-of and type-of relations, synonyms, and hierarchy terms (hypernyms, hyponyms, etc.) from the WordNet synset.

        #### 4. Threat Embracing

        Embrace the candidates that look suspicious, following the embracing operation micro-steps as described below.
        ...
        - Use the threshold and aggregation settings to fine-tune the sensitivity of threat embracing as you like!
        - You can also compute similarity scores using different methods, just make sure the Input Threats CSV and Semantic Similarity Scores CSV follow the same structure as expected by this app. You can see more in the example and result files within the repository.
